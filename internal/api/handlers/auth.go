...
type AuthUserRepo interface {
	Upsert(ctx context.Context, u *repository.User) error
...
func (h *AuthHandler) Callback(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		WriteError(w, http.StatusBadRequest, "BAD_REQUEST", "Invalid Discord user ID")
		return
	}

	// 1. Verify access token with Discord API directly (Server-side validation)
	...
	if err != nil || resp.StatusCode != http.StatusOK {
		WriteError(w, http.StatusUnauthorized, "UNAUTHORIZED", "Failed to verify Discord access token")
		return
	}
	defer resp.Body.Close()

	// 2. Identify which guilds the user has Administrator or Manage Server permissions on
	...
	// 3. Encrypt the access and refresh tokens using AES-GCM
	encAccess, err := crypto.Encrypt([]byte(payload.AccessToken), h.tokenEncKey)
	if err != nil {
		WriteError(w, http.StatusInternalServerError, "INTERNAL_ERROR", "Token encryption failed")
		return
	}
	...
	if err = h.userRepo.Upsert(r.Context(), dbUser); err != nil {
		WriteError(w, http.StatusInternalServerError, "INTERNAL_ERROR", "Failed to upsert user record")
		return
	}
	...
	if err != nil {
		WriteError(w, http.StatusInternalServerError, "INTERNAL_ERROR", "Failed to sign session token")
		return
	}

	// 6. Return HttpOnly session cookie
	http.SetCookie(w, &http.Cookie{
		...
		HttpOnly: true,
		Secure:   true, // SameSite=Lax will send for dashboard navigation
...
func (h *AuthHandler) Me(w http.ResponseWriter, r *http.Request) {
	...
	managedGuilds := middleware.GetManagedGuilds(r.Context())

	if userID == "" {
		WriteError(w, http.StatusUnauthorized, "UNAUTHORIZED", "Active session not found")
		return
	}

	// Fetch fresh info from DB
