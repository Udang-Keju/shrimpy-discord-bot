...
type WelcomeRepository interface {
	...
	Upsert(ctx context.Context, cfg *repository.WelcomeConfig) (*repository.WelcomeConfig, error)
...
func (s *WelcomeService) Get(ctx context.Context, guildID int64) (*repository.WelcomeConfig, error) {
	cfg, err := s.repo.Get(ctx, guildID)
	if err == repository.ErrNotFound {
		return &repository.WelcomeConfig{GuildID: guildID, Enabled: false}, nil
	}
	return cfg, err
}

// Save upserts the welcome configuration.
func (s *WelcomeService) Save(ctx context.Context, cfg *repository.WelcomeConfig) (*repository.WelcomeConfig, error) {
