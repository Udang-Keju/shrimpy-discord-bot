...
erDiagram
    ...
    ticket_panels ||--o{ panel_handler_roles : "has"
    ticket_categories ||--o{ category_handler_roles : "has"
...
-- Roles granted Level 2 dashboard access (console login + config), NOT ticket handling.
-- See panel_handler_roles / category_handler_roles below for who is invited into tickets.
...
-- Roles invited into the Discord channel/thread for any ticket opened from this panel
-- (handling/visibility inside Discord — not dashboard access; see staff_roles above).
...
CREATE INDEX idx_panel_handler_roles_panel ON panel_handler_roles(panel_id);

-- Roles invited into the Discord channel/thread for tickets opened from this specific
-- category, additive to that category's panel_handler_roles.
...
CREATE INDEX idx_category_handler_roles_category ON category_handler_roles(category_id);
...
# Ticket handler roles — who gets invited into the Discord channel/thread created for a
# ticket, to handle it. Distinct from /staff-roles (§4.7), which gate dashboard access.
GET    /api/v1/guilds/:guildId/panels/:panelId/handler-roles                          # List panel-level handler roles
POST   /api/v1/guilds/:guildId/panels/:panelId/handler-roles                          # Add a panel-level handler role
DELETE /api/v1/guilds/:guildId/panels/:panelId/handler-roles/:roleId                  # Remove a panel-level handler role

GET    /api/v1/guilds/:guildId/panels/:panelId/categories/:catId/handler-roles        # List category-level handler roles (additive to the panel's)
POST   /api/v1/guilds/:guildId/panels/:panelId/categories/:catId/handler-roles        # Add a category-level handler role
DELETE /api/v1/guilds/:guildId/panels/:panelId/categories/:catId/handler-roles/:roleId # Remove a category-level handler role
```

**POST /handler-roles body example (same shape for panel- and category-level):**
```json
...
> **v1 decision (USER_JOURNEY §14.8):** participants (`S-05`) are **deferred** — `panel_handler_roles` and `category_handler_roles` ([§3.2](#32-table-definitions-ddl)) already grant every role-holder in those lists access to a ticket's channel/thread automatically (applied as Discord permission overwrites when the ticket is created), covering the real-world need without a new table.
